...
int parseurJson(const char *message, int *position, objetJson *contenu, TYPEJSON type)
{
    ...
    {
        printf("Erreur de typage à l'appel de fonction\n");
        exit(EXIT_FAILURE);
    }
...
void parsingValeurJson(char *message, int *position, void **valeur, TYPEJSON *typeJson)
{
    ...
    case '{': // Cas d'un objet json
        ...
        *valeur = (objetJson *)calloc(1, sizeof(objetJson));
        ...
        parseurJson(message, position, *valeur, OBJET); // On reboucle sur l'intérieur de la valeur json
        break;
...
void lectureparsingJson(objetJson *contenu, char *decalage)
{
    ...
    while (elt != NULL)
    {
        // Affichage de la clé si existante
        if (elt->cle != NULL)
...
// Permet de générer le texte du Json récupéré
void generationTexteJson(objetJson *contenu, char **reponse)
{
    // Initialisation, ne se reproduit pas lors des récursivités
    if (*reponse == NULL)
    {
    ...
    // On supprime le tout dernier caractère qui est une virgule
    (*reponse)[strlen(*reponse)-1] = '\0';
}

// Attention, la recherche va permettre de pointer directement sur l'élément sans en faire une copie
...
int recuperationJsonFichier(objetJson *contenu, char *nomFichier)
{
    ...
    }
    {
        ...
        // On crée la chaine chainée Json
        position = 0;
